...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["username", "password", "email"]  # Include password & email
        extra_kwargs = {"password": {"write_only": True}}  # Prevent password exposure

    def create(self, validated_data):
        """Ensure password is properly hashed when creating a user."""
...
class PostSerializer(serializers.ModelSerializer):
    ...
    items_detail = ItemSerializer(
        many=True, read_only=True
    )  # Used only for GET responses
