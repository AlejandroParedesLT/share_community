                ...
                with open(csv_file_path, "r", encoding=enc) as csv_file:
                    ...
                    for i, row in enumerate(csv_reader):
                        ...
                        try:
                            ...
                            print(
                                f"Error parsing genres for row {i+1} in {media_type} data"
                            )
                            print(f"Genres string: {genres_str}")
                            print(f"Error: {e}")

                    print(
                        f"Successfully read {row_count} rows from {csv_file_path} with encoding {enc}"
                    )
...
def populate_genres_table(auth_token, genre_mapper):
    ...
    for genre, genre_id in all_genres.items():
        # Create form data
        form_data = {"id": (None, str(genre_id)), "name": (None, genre)}
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"Failed to add genre: {genre}. Status code: {response.status_code}"
                )
                print(f"Response: {response.text}")
...
def main():
    ...
    if auth_token:
        ...
        if MOVIE_CSV_PATH:
            ...
            movie_genres = extract_genres_from_csv(
                MOVIE_CSV_PATH, genre_mapper, "movie", MOVIE_GENRE_COLUMN
            )
        ...
        if BOOK_CSV_PATH:
            ...
            book_genres = extract_genres_from_csv(
                BOOK_CSV_PATH, genre_mapper, "book", BOOK_GENRE_COLUMN
            )
        ...
        if MUSIC_CSV_PATH:
            ...
            music_genres = extract_genres_from_csv(
                MUSIC_CSV_PATH, genre_mapper, "music", MUSIC_GENRE_COLUMN
            )
        ...
        for genre, genre_id in sorted(
            [(g, genre_mapper.get_genre_id(g)) for g in all_genres], key=lambda x: x[1]
        ):
