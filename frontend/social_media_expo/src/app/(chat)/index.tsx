...
//         renderItem={({ item }) => {
//           const participantNames = Array.isArray(item.participants_details)
//             ? item.participants_details.map(p => p.username).join(', ')
//             : "Unknown";
...
//         onPress={() => router.push('/(chat)/joinChat')}
//       >
...
export default function ChatsScreen() {
  const [chats, setChats] = useState([]);
  const { accessToken, user } = useAuth();  // Get the access token and user
        ...
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              ...
              />
              <View style={styles.chatInfo}>
                <Text style={styles.name}>{String(participantNames)}</Text>
                <Text style={styles.lastMessage}>{String(item.last_message?.content || "No messages yet")}</Text>
              </View>
            </TouchableOpacity>
          );
        }}
