...
function Posts() {
    ...
    const getInitialsAndColor = (name) => {
        const generateColor = (input) => {
            if (!input) return avatarColors[Math.floor(Math.random() * avatarColors.length)];
            let hash = 0;
        ...
        return { initials: getInitials(), color: generateColor(name) };
    };
    ...
    return (
        <div style={styles.pageContainer}>
            ...
            <PostPostModal isOpen={isModalOpen} onClose={() => setModalOpen(false)} navigate={navigate} />
            ...
            <div style={styles.container}>
                <div style={styles.recentPostsHeader}>What Are Other Slicers Posting About?</div>
                {posts.length === 0 ? (
                    <div style={styles.noPostsMessage}>No posts found or loading...</div>
                ) : (
                    <div style={styles.gridContainer}>
                        {posts.map((post) => {
                            const { initials, color } = getInitialsAndColor(post.user?.username || post.user?.email || 'Unknown');
                            return (
                                <div 
                                    key={post.id}
                                    style={styles.postCard}
                                    onClick={(event) => handlePostCardClick(event, post)}
                                    onMouseEnter={(e) => e.currentTarget.style.transform = styles.postCardHover.transform}
                                    onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
                                >
                                    {post.presigned_image_url && (
                                        <img src={post.presigned_image_url} alt={post.title} style={styles.postImage} />
                                    )}
                                    <div style={styles.postContent}>
                                        <div style={styles.userInfo}>
                                            <div style={{ ...styles.userAvatar, backgroundColor: color }}>{initials}</div>
                                            <p style={{ margin: 0, fontWeight: 'bold', fontSize: '14px' }}>
                                                {post.user?.username || post.user?.email || 'Unknown User'}
                                            </p>
                                        </div>
                                        <h3 style={styles.postTitle}>{post.title}</h3>
                                        <p style={styles.postMeta}>Created on: {new Date(post.created_at).toLocaleString()}</p>
                                        {post.content && <p style={styles.postText}>{post.content}</p>}
                                        {post.items && post.items.length > 0 && (
                                            <div>
                                                <h4 style={styles.associatedItemsTitle}>Associated Items:</h4>
                                                {post.items.map((item) => (
                                                    <div 
                                                        key={item.precordsid} 
                                                        style={styles.associatedItem}
                                                        onClick={(event) => handleAssociatedItemClick(event, item)}
                                                        onMouseEnter={(e) => e.currentTarget.style.transform = styles.associatedItemHover.transform}
                                                        onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
                                                    >
                                                        <span>{item.title}</span>
                                                        {item.presigned_image_url && (
                                                            <img src={item.presigned_image_url} alt={item.title} style={styles.associatedItemImage} />
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
